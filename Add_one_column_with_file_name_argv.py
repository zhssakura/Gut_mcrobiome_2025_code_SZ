...
parser.add_argument('-E.g',
                    required=False,
                    help='python3 Add_one_column_with_file_name_argv.py -dir -format')

parser.add_argument('-dir',
                    required=True,
                    help='./Documents/07_HKU/03_Research/01_PRJ_NAFLD_gut_microbiome/1_Databases/European_Nucleotide_Archive_ENA/PRJEB55534/8_gapseq/')

parser.add_argument('-format',
                    required=True,
                    default='Pathways.tbl',
...
for file_name in files:
    ...
    mag_ID = file_name.strip().split("-")[0]
    sample_ID = mag_ID.split("_bin")[0]
    # print(mag_ID)
    outfile = ('%s%s%s' % (path, file_name_prefix, "_MAG_added.tsv"))
    # print(outfile)
    # print('\n')
    ...
    for each in open('%s%s' % (path, file_name)):
        ...
        if each.startswith("ID	Name	"):
            new_header = "ID	Name	Prediction	Completeness	VagueReactions	KeyReactions	KeyReactionsFound	ReactionsFound\tMagID\tSampleID\n"
            output_handel.write(new_header)
        if each.startswith("|"):
            # print(len(each_split))
            if len(each_split)==8:
                new_each = '%s\t%s\t%s\n' % ('\t'.join(each_split), mag_ID,sample_ID)
                output_handel.write(new_each)
            else:
                new_each = '%s\t%s\t%s\t%s\n' % ('\t'.join(each_split), "NA", mag_ID,sample_ID)
                output_handel.write(new_each)
    output_handel.close()
print("Done! you have the output files named *_MAG_added.tsv")
